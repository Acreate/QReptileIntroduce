...
HtmlDocString RequestNet::formHtmlGetNext( const interfacePlugsType::HtmlDocString &type_name, const interfacePlugsType::HtmlDocString &url, const HtmlDocString &htmlText, const Vector_INovelInfoSPtr &saveNovelInfos, const Vector_INovelInfoSPtr &lastNovelInfos ) {
	...
	std::shared_ptr< HtmlNode > htmlNode = htmlNodes->at( 0 );
	HtmlString_Shared contentText = htmlNode->getNodeIncludeContentText( );
	if( contentText == nullptr )
		return result;
	auto number = QString::fromStdWString( *contentText );

	bool conver = false;
	qulonglong pageCount = number.toULongLong( &conver );
	if( conver == false )
		return result;
	++pageCount;
	auto newNextPage = QString::fromStdWString( url );
	qsizetype indexOf = newNextPage.lastIndexOf( "_" );
	if( indexOf == -1 )
		return result;
	newNextPage = newNextPage.mid( 0, indexOf + 1 ) + QString::number( pageCount ) + ".html";
	result = newNextPage.toStdWString( );
	return result;
}
